#!/usr/bin/python2
# -*- coding: utf-8 -*- crowdin_sync.py
#
# Updates Crowdin source translations and pushes translations directly to OmniROM's Gerrit.
#
# Copyright (C) 2014 The CyanogenMod Project Modifications Copyright (C) 2014 OmniROM
#
...
# under the License.
############################################# IMPORTS ##############################################
import argparse import codecs import git import os import os.path import re import shutil import subprocess import sys from urllib import urlretrieve 
...
def push_as_commit(path, name, branch, username):
    ...
    try:
        ...
        return
    # Push commit
    repo.git.push('ssh://' + username + '@gerrit.omnirom.org:29418/' + name, 'HEAD:refs/for/' + branch + '%topic=translation')
    print('Succesfully pushed commit for ' + name)
###################################################################################################
print('Welcome to the OmniROM Crowdin sync script!')
###################################################################################################
parser = argparse.ArgumentParser(description='Synchronising OmniROM\'s translations with Crowdin') parser.add_argument('--username', help='Gerrit 
username', required=True)
#parser.add_argument('--upload-only', help='Only upload OmniROM source translations to Crowdin', required=False)
args = vars(parser.parse_args()) username = args['username']
############################################## STEP 0 ##############################################
...
if subprocess.check_output(['rvm', 'all', 'do', 'gem', 'list', 'crowdin-cli', '-i']) == 'true':
    sys.exit('You have not installed crowdin-cli. Terminating.') else:
    print('Found: crowdin-cli')
# Check for repo
try:
    subprocess.check_output(['which', 'repo']) except:
    sys.exit('You have not installed repo. Terminating.')
# Check for android/default.xml
if not os.path.isfile('android/default.xml'):
    sys.exit('You have no android/default.xml. Terminating.') else:
    print('Found: android/default.xml')
# Variables regarding android/default.xml
print('Loading: android/default.xml') xml_android = minidom.parse('android/default.xml')
# Default branch
default_branch = 'android-4.4' print('Default branch: ' + default_branch)
# Check for crowdin/extra_packages.xml
if not os.path.isfile('crowdin/extra_packages_' + default_branch + '.xml'):
    sys.exit('You have no crowdin/extra_packages_' + default_branch + '.xml. Terminating.') else:
...
if not os.path.isfile('crowdin/config.yaml'):
    sys.exit('You have no crowdin/config.yaml. Terminating.') else:
...
print(subprocess.check_output(['crowdin-cli', '--config=crowdin/crowdin_crowdin_' + default_branch + ', '--identity=crowdin/config.yaml', 'upload', 
'sources']))
############################################## STEP 2 ##############################################
print('\nSTEP 2: Download Crowdin translations')
# Execute 'crowdin-cli download' and show output
print(subprocess.check_output(['crowdin-cli', '--config=crowdin/crowdin_' + default_branch + ', '--identity=crowdin/config.yaml', 'download']))
############################################## STEP 3 ##############################################
print('\nSTEP 3: Remove useless empty translations')
# Some line of code that I found to find all XML files
result = [os.path.join(dp, f) for dp, dn, filenames in os.walk(os.getcwd()) for f in filenames if os.path.splitext(f)[1] == '.xml'] empty_contents = 
{'<resources/>', '<resources xmlns:android="http://schemas.android.com/apk/res/android"/>', '<resources 
xmlns:xliff="urn:oasis:names:tc:xliff:document:1.2"/>', '<resources xmlns:android="http://schemas.android.com/apk/res/android" 
xmlns:xliff="urn:oasis:names:tc:xliff:document:1.2"/>'} for xml_file in result:
...
proc = subprocess.Popen(['crowdin-cli --config=crowdin/crowdin_' + default_branch + ' --identity=crowdin/config.yaml list sources | grep "' + 
default_branch + '" | sed "s#/' + default_branch + '##g"'], stdout=subprocess.PIPE, shell=True) proc.wait() # Wait for the above to finish
############################################## STEP 5 ##############################################
print('\nSTEP 5: Commit to Gerrit') xml_extra = minidom.parse('crowdin/extra_packages_' + default_branch + '.xml') items = 
xml_android.getElementsByTagName('project') items += xml_extra.getElementsByTagName('project') all_projects = [] for path in 
iter(proc.stdout.readline,''):
    ...
    if result in all_projects:
        ...
        continue
    # When a project has multiple translatable files, Crowdin will give duplicates. We don't want that (useless empty commits), so we save each 
