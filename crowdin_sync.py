...
print(subprocess.check_output(['crowdin-cli', '--config=crowdin/crowdin_crowdin_' + default_branch + ', '--identity=crowdin/config.yaml', 'upload', 'sources']))
...
proc = subprocess.Popen(['crowdin-cli --config=crowdin/crowdin_' + default_branch + ' --identity=crowdin/config.yaml list sources | grep "' + default_branch + '" | sed "s#/' + default_branch + '##g"'], stdout=subprocess.PIPE, shell=True) proc.wait() # Wait for the above to finish
